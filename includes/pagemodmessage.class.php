...
class PageModMessage extends Page {
    ...
    public function init() {
        ...
        set_include_path ( 
                get_include_path () . ":$tableauRootDir/tableau/jHtmlArea-0.7.5/scripts/:$tableauRootDir/tableau/jquery-ui-1.9.0.custom/js/:$tableauRootDir/tableau/jHtmlArea-0.7.5/style/:$tableauRootDir/tableau/:jquery-ui-1.9.0.custom/css/smoothness/" );
    ...
    public function recevoirLesDonnees() {
        ...
        if ($this->getRequestParameter ( 'ajax' )) {
            $output ['success'] = 1;
            $editedMessages = json_decode ( $this->getRequestParameter ( 'editedMessages' ),"{}" );
            
            if (is_array ( $editedMessages )) {
                $firePHP->log($editedMessages,'Edited Messages') ;
                foreach ( $editMessages as $editedMessage ) {
                    $messageId = null ;
                    if(isset($editedMessage ['id'])) $messageId = $editedMessage ['id'] ;
                    $message = $this->lesMessages->getUnMessage ( $messageId );
                    if (isset ( $editedMessage ['selected'] ) && $message->isLoaded()) {
                        $message->delete ();
                    } else {
            ...
            $nouveauMessage = json_decode($this->getRequestParamenter("nouveauMessage"),"{}") ;
            if(is_array($nouveauMessage)) {
...
<form action="modmessage.php" method="post">
	<input type="hidden" name="messageId"
		value="<?php echo $this->monMessage->getId()?>"></input>
	<div class="row">
	...
	<div class="row">
		<div class="column grid_2">&nbsp;</div>
		<div class="column grid_4 valeurItem">
			<input type="text" name="debut"
				value="<?php echo $this->monMessage->getDebut(true)?>" size="10"
				class="dateInput"></input>
		</div>
		<div class="column grid_4 valeurItem">
			<input type="text" name="fin"
				value="<?php echo $this->monMessage->getFin(true)?>" size="10"
				class="dateInput">
		</div>
	</div>
	<div class="row">
		<div class="column grid_2">&nbsp;</div>
		<div class="column grid_13 nomItem">
			<label for="titreMessage">Titre </label> <input id="titreMessage"
				type="text" name="titre"
				value="<?php echo $this->monMessage->getTitre()?>" size="100"></input>
		</div>
	</div>
	<div class="row">
		<div class="column grid_2">&nbsp;</div>
		<div class="column grid_13 nomItem">
			<textarea id="messageBox" name="message" cols="80" rows="20"><?php echo $this->monMessage->getMessage()?></textarea>
		</div>
	</div>
	<div class="row">
		<div class="column grid_2">&nbsp;</div>
		<div class="column grid_8 valeurItem">
			<input type="reset" name="reset" value="annuler les modifications"></input>&nbsp;
			<input type="submit" name="submit"
				value="sauvegarder les changements"></input>
		</div>
	</div>
</form>
<?php
